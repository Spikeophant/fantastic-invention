...
const mainPageRender = function() {
  for (let x = 7; x<19; x++) {
    ...
    if (Number(moment().hour(x).format('H')) < Number(moment().format('H'))) {
      taskCol = $('<div class="col-10 time-block past"><textarea class="col-10"  id="Text' + x +'"></textarea></div>');
    } else if (moment().hour(x).format('H') === moment().format('H')) {
      taskCol = $('<div class="col-10 time-block present"><textarea class="col-10" id="Text' + x +'"></textarea></div>');
    } else {
      taskCol = $('<div class="col-10 time-block future"><textarea class="col-10" id="Text' + x +'"></textarea></div>');
    }
